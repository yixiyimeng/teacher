<template>
	<div>
		...
		<div class="bottommenu">
			...
			<a href="javascript:;" class="start" @click="startRace" v-show="isSubject"></a>
			...
			<a href="javascript:;" class="next" @click="nextQuestion">
				<!-- <span>下一题</span> -->
				<i></i>
			</a>
		</div>
		...
		<!-- 选择答案的名单 -->
		<div class="namelistbox animated fast fadeIn" :class="[isshowselectNamelist ? 'fadeIn' : 'fadeOut']" v-if="isshowselectNamelist">
			<a href="javascript:;" class="colse" @click="isshowselectNamelist = !isshowselectNamelist">×</a>
			<ul class="clearfix">
				<!-- {{namelist}} -->
...
export default {
	...
	data() {
		return {
			...
			selectNamelist: []
			// titlename:['单题单选','判断题','单题多选','主观题','抢红包','语音识别','语音测评', '麦克风']
		};
	},
	computed: {
		...
		nextQuestion() {
			...
			$me.$http({
				...
				if (da.data.ret == 'success') {
					$me.clear();
					$me.isAnswering = true; //开始答题
					$me.subjecttitle = da.data.data.questionType;
					$me.trueAnswer = da.data.data.trueAnswer;
					var titlenamelist = [...$me.subjectitleList, ...$me.yysubjectitleList];
					console.log(titlenamelist);
					$me.titlename = titlenamelist[$me.subjecttitle - 1].name;
					/*开始答题*/
					if ($me.subjecttitle != 6 && $me.subjecttitle != 7 && $me.subjecttitle != 8) {
						$me.isprogress = true; //显示进度条
					}
