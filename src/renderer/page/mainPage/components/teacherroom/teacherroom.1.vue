<template>
	<div style="height: 100%; overflow: hidden;">
		...
		<div class="bottommenu">
			...
			<a href="javascript:;" class="prev" @click="prevQuestion" v-show="isSubject && !isAddSubject"><i></i></a>
		...
		<!-- 左侧菜单 -->
		<div class="leftmenu">
			<a href="javascript:;" @click="isshowNamelist = !isshowNamelist" :class="{ active: isshowNamelist }">
				<i class="icon1"></i>
				学生名单
			</a>
			<a href="javascript:;" @click="showResource(1)" :class="{ active: isshowResource == 1 }">
				<i class="icon2"></i>
				学科网
			</a>
			<a href="javascript:;" @click="showResource(2)" :class="{ active: isshowResource == 2 }">
				<i class="icon3"></i>
				组卷网
			</a>
		...
		<count-down v-if="isCountDown" v-show="isAnswering" :setTimer="countDown * 1000" @stopCountDown="stopCountDown" ref="countdown"></count-down>
		...
		<audiolist :selectWordList="audiohistorylist"></audiolist>
	</div>
</template>
...
import { notice, progressbox, dropmenu, search, load, board, timeswiper, CountDown, toolbar, audiotxt, audiolist } from '@/page/mainPage/components';
...
export default {
	...
	methods: {
						...
						if (received_msg != '连接成功') {
							...
							switch (msg.reqType) {
								...
								case 7: {
									...
									// 优秀（90-100）、良好（80-90）、中等（70-80）、继续努力（70以下）评价
									var score = obj.score;
									var answer = '';
									if (score >= 90) {
										answer = '非常棒';
			...
			this.$http({
				...
				.then(da => {
					...
					if ($me.subjecttitle == 9) {
						...
						if (($me.audiohistorylist.length > 0 && !$me.audiohistorylist.some(item => item.wordtxt == wordtxt)) || $me.audiohistorylist.length == 0) {
							$me.audiohistorylist.push({
								wordtxt: wordtxt,
		...
		// 		console.log(da)
		// 		if (da.data && da.data.ret == 'success') {
		// 			this.audiohistorylist= da.data.data;
		// 		} else {
